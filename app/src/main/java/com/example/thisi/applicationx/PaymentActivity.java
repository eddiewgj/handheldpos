...
public class PaymentActivity extends Activity {
    ...
    private void insertPaymentsIntoDb() {
        ...
        try {
            ...
            String rcp_id = insertHeader(db, currentShift.SHIFT_NUMBER);// Edited by Eddie 14/12/2016, added Shift
    ...
    private String insertHeader(SQLiteDatabase db, int shiftNo) { // Edited by Eddie 14/12/2016, add the shift parameter

        // Edited by Eddie 14/12/2016, generate receipt number, begin
        String receiptNo = "";
        String queryLastReceiptNo = "SELECT last_rcp FROM pos_control WHERE company_code = '" + companyCode + "' AND outlet_code = '" + outletCode + "' AND pos_no = '" + posNo + "';";
        Cursor curs = db.rawQuery(queryLastReceiptNo, null);

        if (curs.getCount() < 0){
            String insertNewPOSCOntrol = "INSERT INTO pos_control (COMPANY_CODE, OUTLET_CODE, POS_NO, BUS_DATE, SHIFT_NUMBER, EMP_CD, LAST_RCP, LAST_SUSPEND_NUMBER, REPRINT_COUNT, DAYEND) " +
                    "SELECT shift_master.COMPANY_CODE, shift_master.OUTLET_CODE, shift_master.POS_NO, shift_master.BUS_DATE, '" + shiftNo + "', '" + empCode + "', '0000000', '0000000', '0', '0'" +
                    "FROM shift_master WHERE shift_master.SHIFT_STATUS = '/???/'" ;
            db.execSQL(insertNewPOSCOntrol);
            curs = db.rawQuery(queryLastReceiptNo, null);
        }
        ...
        // Edited by Eddie 14/12/2016, end
        ...
        // Edited by Eddie 14/12/2016, update last receipt number to pos control
        String updatePOSControl = "UPDATE pos_control SET " +
                "last_rcp = '" + receiptNo + "', " +
                "emp_cd = '" + empCode + "', " +
                "bus_date = '" + todaysDateInString + "', " +
